...
| 3 | LLM via gateway.route(capability='llm') | `app/gateway/gateway.py` | Render `script-refine.v1.0.0.yaml` prompt; DeepSeek V3.2 via gateway pool; 15s timeout | ProviderUnavailableError → 503; turn NOT counted; NO COGS |
...
|---|---|---|
|Gate 1 — Foundation Sign-Off|After §11 schema, §7 FSM, §10 design tokens are wired|(i) `alembic upgrade head` clean on a fresh DB; (ii) seeded ENUMs match `[TDD-ENUMS]`; (iii) audit_log REVOKE policy verified by attempting an UPDATE as the app role and confirming permission denied; (iv) CI-A green.|
...
| --- | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- | ----------------------------------------- |
| 1   | Days 1–3 (May 1-3)     | **Foundation** — repo scaffold, design tokens, FSM seed, schema migration v1, ENUM seed, audit_log REVOKE policy                                                        | §7, §10, §11                                                                        | Gate 1 cleared                            |
